function plot_cmpr_int(compare,interest,color,class,sw,y_lim,xtick,chance) %sw for line
...
figure2 = figure('InvertHardcopy','off','PaperUnits','centimeters',...
    'Color',[1 1 1],...
    'Renderer','painters','position',[100 100 200+size(interest,1)*30 200]);
...
datum=nan(size(compare{interest(1,1),interest(1,2)},1),2*size(interest,1));
...
for i=1:size(datum,1)
    for j=1:length(G)/2
    plot(G(2*j-1:2*j),100*datum(i,2*j-1:2*j),'-o','color',[0.7 0.7 0.7],'markersize',4)
    hold all
    end
end
